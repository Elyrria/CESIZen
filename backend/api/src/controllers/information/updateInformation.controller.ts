...
export const updateInformation = async (req: IAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (req.body.descriptionInformation)
      updateData.descriptionInformation = req.body.descriptionInformation
      ...
      try {
        ...
        if (oldFileId) {
          try {
            ...
            logger.info(`Successfully deleted old file: ${chalk.green(oldFileId.toString())}`)
          } catch (deleteError) {
            // Log error but continue with update
            logger.error(`Failed to delete old file: ${chalk.red(oldFileId.toString())}`)
          }
        }
      } catch (uploadError) {
    ...
    if (
      [MEDIATYPE[1], MEDIATYPE[2]].includes(updatedInformation.type) &&
      updatedInformation.fileId
    ) {
