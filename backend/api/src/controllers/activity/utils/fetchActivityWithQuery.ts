 ...
 */
export const fetchActivityWithQuery = async (
  req: Request,
  baseQuery: mongoose.FilterQuery<IActivityDocument> = {}
) => {
  ...
  const userQuery = buildActivityQuery(req)
  ...
  if (categoryInfo) {
    // @ts-expect-error - Adding category property to response
