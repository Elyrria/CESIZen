...
if (ENCRYPTION_KEY.length !== CRYPTO.keyLength) {
  throw new Error(
    `Invalid encryption key length. Must be ${CRYPTO.keyLength} bytes for ${CRYPTO.algorithm}.`
  )
}
 ...
 */
export async function processUserUpdateData(
...
export async function processRefreshToken(
  refreshTokenData: IRefreshTokenCreate
): Promise<IRefreshTokenCreate> {
...
export function decryptData(
  userData: Record<string, any> | Record<string, any>[],
