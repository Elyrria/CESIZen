...
export function validateRequiredUserFields(user: Partial<IUserCreate>): boolean {
  return Boolean(
    user.role && user.password && user.email && user.name && user.firstName && user.birthDate
  )
}

export function validateRequierdInformationFields(
  information: Partial<IInformationDocument>
): boolean {
  return Boolean(
    information.title && information.descriptionInformation && information.name && information.type
  )
}
