...
export const SHARED_MESSAGES = {
  ...
  statusInvalid: (roles: string[]): string =>
    `Le statut doit être l'un des suivants : ${roles.join(", ")}`,
  minLength: (type: string, min: number): string =>
    `Le ${type} doit contenir au moins ${min} caractères`,
  maxLength: (type: string, max: number): string =>
    `Le ${type} ne doit pas contenir plus de ${max} caractères`
}
export const ACTIVITY_MESSAGE = {
  ...
  typeInvalid: (type: string[]): string =>
    `Le type doit être l'un des suivants : ${type.join(", ")}`,
...
export const USER_MESSAGE = {
  ...
  roleInvalid: (roles: string[]): string =>
    `Le rôle doit être l'un des suivants : ${roles.join(", ")}`
}
...
export const TOKEN_MESSAGE = {
  ...
  invalidFormat: (type: string): string =>
    `Le ${type} doit avoir un format valide : MongoDB ObjectID`,
