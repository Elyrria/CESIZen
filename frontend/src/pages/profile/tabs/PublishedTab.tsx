    ...
    },
    {
      ...
      cell: (item) => (
        <div className="flex gap-2">
          ...
          <button
            onClick={() =>
              navigator.clipboard.writeText(`${window.location.origin}/informations/${item.id}`)
            }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="font-medium text-green-800 mb-1">Informations publiées</h4>
            <p className="text-sm text-green-700">
              Ces informations sont maintenant visibles par tous les utilisateurs. Elles sont
              accessibles depuis la page publique des informations.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-fr-grey-light p-4 rounded-lg">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-fr-grey-dark">Total publié :</span>
                <span className="font-medium">{publishedInformations.length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-fr-grey-dark">Type TEXT :</span>
                <span className="font-medium">
                  {publishedInformations.filter((i) => i.type === "TEXT").length}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-fr-grey-dark">Type IMAGE :</span>
                <span className="font-medium">
                  {publishedInformations.filter((i) => i.type === "IMAGE").length}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-fr-grey-dark">Type VIDEO :</span>
                <span className="font-medium">
                  {publishedInformations.filter((i) => i.type === "VIDEO").length}
                </span>
              </div>
            </div>
          </div>
              ...
              <button
                onClick={() => {
                  const links = publishedInformations
                    .map((info) => `${window.location.origin}/informations/${info.id}`)
                    .join("\n")
        ...
        <div className="bg-white border border-fr-grey-light rounded-lg p-4">
          ...
          <div className="space-y-2">
            {publishedInformations
              .sort(
                (a, b) =>
                  new Date(b.validatedAndPublishedAt || "").getTime() -
                  new Date(a.validatedAndPublishedAt || "").getTime()
              )
              .slice(0, 3)
              .map((item) => (
                <div
