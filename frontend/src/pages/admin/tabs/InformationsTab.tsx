  ...
  const informationColumns: ColumnDef<IInformation>[] = [
    {
      ...
      cell: (item: IInformation) => (
        <div>
          ...
          <p className="text-sm text-fr-grey-dark capitalize">{item.type.toLowerCase()}</p>
        </div>
      )
    },
    {
        ...
        <div className="flex flex-wrap gap-1">
          {Array.isArray(item.categoryId) ? (
            item.categoryId.slice(0, 2).map((cat, index) => (
              <span key={index} className="px-2 py-1 bg-fr-blue-light text-fr-blue text-xs rounded">
                {typeof cat === "string" ? cat : isCategoryObject(cat) ? cat.name : "Catégorie"}
              </span>
            ))
          ) : (
    ...
    },
    {
      ...
      cell: (item: IInformation) => (
        <div className="flex gap-2 items-center">
          ...
          <select
            value={item.status}
            onChange={(e) => handleStatusChange(item.id, e.target.value as InformationStatus)}
