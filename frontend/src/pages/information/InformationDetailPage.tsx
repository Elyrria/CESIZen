    ...
    if (currentInfo.type === "VIDEO") {
      return (
        <div className="aspect-w-16 aspect-h-9">
          <video controls className="w-full h-full rounded-lg" src={currentInfo.mediaUrl || ""}>
      ...
      <article className="bg-white rounded-lg shadow-fr-md p-6">
        <header className="mb-6">
          ...
          <div className="flex items-center space-x-4 text-fr-grey-dark">
            <StatusBadge status={currentInfo.status as "DRAFT" | "PENDING" | "PUBLISHED"} />
            {categoryName && <span className="text-sm">Category: {categoryName}</span>}
          </div>
        </header>

        <div className="mb-6">
          <p className="text-lg text-fr-grey-dark">{currentInfo.descriptionInformation}</p>
        </div>
